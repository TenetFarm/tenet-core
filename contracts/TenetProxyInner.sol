...
contract TenetProxyInner is Ownable {
    ...
    mapping (address => SwapFactoryInfo) public allFactoryInfo;
    ...
    constructor(Tenet _tenet,address _weth,address _wusdt) public {
        ...
        addFactory(address(uniFactory),_weth,_wusdt);
    }
    function set_tenet(Tenet _tenet) public onlyOwner {
        tenet = _tenet;
        tenetmine = tenet.tenMineCalc();
    }    
    function addFactory(address _factory,address _weth,address _wusdt) public onlyOwner {
        SwapFactoryInfo storage factoryInfo = allFactoryInfo[_factory];
        factoryInfo.weth = _weth;
        factoryInfo.wusdt = _wusdt;        
    }
    ...
    function calcPrice(IUniswapV2Factory _factory,address tokenAddr) public view returns (uint256,uint256) {
        SwapFactoryInfo storage factoryInfo = allFactoryInfo[address(_factory)];
        uint256 price0 = calcTokenWealth(_factory,tokenAddr,factoryInfo.weth);
        uint256 price1 = calcTokenWealth(_factory,tokenAddr,factoryInfo.wusdt);
    ...
    function calcTokenPrice(IUniswapV2Factory _factory,address token0Addr,address token1Addr) public view returns (uint256,uint256,uint256) {
        ...
        uint256 price0 = 0;
        uint256 price1 = 0;     
        price0 = calcTokenWealth(_factory,token0Addr,factoryInfo.weth);
        if(price0 == 0){
            price1 = calcTokenWealth(_factory,token1Addr,factoryInfo.weth);
            if(price1 == 0){
                pricetype = 1;
                price0 = calcTokenWealth(_factory,token0Addr,factoryInfo.wusdt);
                if(price0 == 0){
                    price1 = calcTokenWealth(_factory,token1Addr,factoryInfo.wusdt);
                }
            }
        }
    ...
    function calcETHPrice() public view returns (uint256) {
        SwapFactoryInfo storage factoryInfo = allFactoryInfo[address(uniFactory)];
        IUniswapV2Pair pair = IUniswapV2Pair(uniFactory.getPair(factoryInfo.weth,factoryInfo.wusdt));
        if (address(pair) == address(0)) {
            return 0;
        }
        address token0 = pair.token0();
        (uint reserve0, uint reserve1,) = pair.getReserves();
        if(token0 == factoryInfo.weth){
